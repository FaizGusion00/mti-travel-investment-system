...
class _WithdrawScreenState extends State<WithdrawScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _searchUser(String query) async {
    ...
    try {
      ...
      if (response['success'] && response['users'] != null) {
        ...
        // Get current user profile to filter out self-transfers
        final profileResponse = await ApiService.getProfile();
        String currentUserEmail = "";
        if (profileResponse['success'] == true && profileResponse['user'] != null) {
          currentUserEmail = profileResponse['user']['email']?.toString() ?? '';
        }
        
        // Filter out current user from search results
        final filteredUsers = List<Map<String, dynamic>>.from(users).where((user) {
          final userEmail = user['email']?.toString() ?? '';
          return userEmail.isNotEmpty && userEmail != currentUserEmail;
        }).toList();
  ...
  // Build withdraw tab with "Coming Soon" message
